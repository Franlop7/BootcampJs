...
import { mapPostPropertyFromToVM } from './upload-property.mappers';
...
Promise.all([getEquipmentsList(), getSaleTypeList(), getProvinceList()]).then(
  ([equipmentsList, saleTypeList, provincesList]) => {
...
onSubmitForm('save-button', () => {
  formValidation.validateForm(newProperty).then((result) => {
    ...
    if (result.succeeded) {
      const ApiUploadProperty = mapPostPropertyFromToVM(newProperty);
      console.log(ApiUploadProperty);
      postPropertyList(ApiUploadProperty).then((res) => {
        history.back();
      });
    }
  });
